...
1. **UPDATE THE SESSION LOG**: Append a new `# Session [N]` entry to the *current day's session file* in `spec/sessions/`. If no file exists for today, you must create one by copying the exact LLM Context & Session templates from the previous day's log. 
